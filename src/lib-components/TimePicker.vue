	...
	computed: {
		opts() {
			...
			if (this.minuteRange && Array.isArray(this.minuteRange)) {
				options.minuteRange = JSON.parse(
			...
			if (this.secondRange && Array.isArray(this.secondRange)) {
				options.secondRange = JSON.parse(
		...
		minuteRangeList() {
			...
			this.opts.minuteRange.forEach(value => {
				if (value instanceof Array) {
					if (value.length > 2 && this.debugMode) {
						this.debugLog(
							`Nested array within "minute-range" must contain no more than two items. Only the first two items of ${JSON.stringify(
								value
							)} will be taken into account.`
						);
					}
			...
			// Debug Mode
			if (this.debugMode) {
				const validItems = (this.minutes || []).filter(item =>
					range.includes(item)
				);
		...
		secondRangeList() {
			...
			this.opts.secondRange.forEach(value => {
				if (value instanceof Array) {
					if (value.length > 2 && this.debugMode) {
						this.debugLog(
							`Nested array within "second-range" must contain no more than two items. Only the first two items of ${JSON.stringify(
								value
							)} will be taken into account.`
						);
					}
			...
			// Debug Mode
			if (this.debugMode) {
				const validItems = (this.seconds || []).filter(item =>
					range.includes(item)
				);
				if (!validItems || !validItems.length) {
					this.debugLog(
	...
	methods: {
		...
		renderList(listType, interval) {
			...
			if (listType === "minute") {
				interval =
					...
					DEFAULT_OPTIONS.minuteInterval;
			} else {
				interval =
					interval ||
					this.opts.secondInterval ||
					DEFAULT_OPTIONS.secondInterval;
			}
			const result = [];
		...
		readStringValues(stringValue) {
			...
			const needsPolyfill = Boolean(
				!formatString.matchAll ||
					typeof formatString.matchAll !== "function"
			);
			const tokensMatchAll = needsPolyfill
				? this.matchAllPolyfill(formatString, tokensRegxStr)
				: formatString.matchAll(new RegExp(tokensRegxStr, "g"));
			const othersMatchAll = needsPolyfill
				? this.matchAllPolyfill(formatString, othersRegxStr)
				: formatString.matchAll(new RegExp(othersRegxStr, "g"));
			const chunks = [];
			const tokenChunks = [];
			...
			chunks.forEach(chunk => {
				if (chunk.isValueToken) {
					...
					const safeChars = chunk.token.replace(
						/\\{0}(\*|\?|\.|\+)/g,
						"\\$1"
					);
			...
			if (comboReg.test(stringValue)) {
				...
				const valueResults = matchResults.slice(
					1,
					tokenChunks.length + 1
				);
				const timeValue = {};
				valueResults.forEach((value, vrIndex) => {
					if (tokenChunks[vrIndex]) {
						const tokenType = tokenChunks[vrIndex].token;
						timeValue[tokenType] = this.setValueFromString(
							value,
							tokenType
						);
					}
				...
				matchesList.forEach(matchedItem => {
					...
					if (existIndex >= 0) {
						if (
							...
							indicesReg[existIndex].regex
						) {
							index = indicesReg[existIndex].regex.exec(
								targetString
							).index;
						}
					} else {
		...
		setValueFromString(parsedValue, tokenType) {
			...
			switch (tokenType) {
				...
				case `${this.minuteType}`:
					stdValue =
						parsedValue !== this.minuteType ? parsedValue : "";
					this.minute = stdValue;
					break;
				case `${this.secondType}`:
					stdValue =
						parsedValue !== this.secondType ? parsedValue : "";
		...
		fillValues() {
			...
			const hourValue =
				baseHour || baseHour === 0 ? Number(baseHour) : "";
			const apmValue =
		...
		updateTimeValue(fullVals) {
			...
			const baseTimeValue = JSON.parse(
		...
		doubleCheckHourValue() {
			...
			const hourIn24 = this.translate12hRange(
				`${this.hour}${this.apm.substr(0, 1)}`
			);
			if (!this.hourRangeIn24HrFormat.includes(hourIn24)) {
				this.hour = "";
				if (this.debugMode) {
					this.debugLog(
						`After switching AM/PM, the hour value is no longer in the "hour-range". Reset it to empty now.`
					);
				}
			}
		},
		...
		getSideItems(columnClass, dataKey, getPrevious = false) {
			...
			const selfIndex = Array.prototype.findIndex.call(
				siblingsInCol,
				sbl => {
			...
			if (getPrevious && selfIndex === 0) {
				if (this.debugMode) {
					this.debugLog(
						`"${dataKey}" is the first valid item in "${columnClass}" list already`
					);
				}
				return;
			}
			...
			if (!getPrevious && selfIndex === siblingsInCol.length - 1) {
				if (this.debugMode) {
					this.debugLog(
						`"${dataKey}" is the last valid item in "${columnClass}" list already`
					);
				}
				return;
			}
		...
		getFirstItemInSideColumn(columnClass, toLeft = false) {
			const targetColumnClass = this.getSideColumnClass(
				columnClass,
				toLeft
			);
		...
		toRightColumn(columnClass) {
			const targetItem = this.getFirstItemInSideColumn(
				columnClass,
				false
			);
			if (targetItem) {
	...
	<div>
		...
		<span class="vue__time-picker time-picker">
			<input
				...
				:class="[inputClass, {'disabled': disabled},{ 'border-red-400': errors.length }]"
				:id="id"
				:name="name"
			...
			>
				<path d="M11,2A10,10,0,1,0,21,12,10,10,0,0,0,11,2Zm0,18a8,8,0,1,1,8-8A8,8,0,0,1,11,20ZM11,6a1,1,0,0,0-1,1v4H8a1,1,0,0,0,0,2h3a1,1,0,0,0,1-1V7A1,1,0,0,0,11,6Z"></path>
			</svg>
			<!-- <svg
				class="absolute text-gray-400 fill-current"
				width="20"
			...
			<span
				class="clear-btn"
				v-if="!showDropdown && showClearBtn"
				@click="clearTime"
			>&times;</span>
			<div
					...
					<template v-if="!advancedKeyboard">
						<ul class="hours">
							...
							<template v-for="(hr, hIndex) in hours">
								<li
									v-if="!opts.hideDisabledHours || (opts.hideDisabledHours && !isDisabledHour(hr))"
									:key="hIndex"
						...
						<ul class="minutes">
							...
							<template v-for="(m, mIndex) in minutes">
								<li
									v-if="!opts.hideDisabledMinutes || (opts.hideDisabledMinutes && !isDisabledMinute(m))"
									:key="mIndex"
						...
						>
							...
							<template v-for="(s, sIndex) in seconds">
								<li
									v-if="!opts.hideDisabledSeconds || (opts.hideDisabledSeconds && !isDisabledSecond(s))"
									:key="sIndex"
						...
						>
							...
							<template v-for="(a, aIndex) in apms">
								<li
									v-if="!opts.hideDisabledHours || (opts.hideDisabledHours && !isDisabledApm(a))"
									:key="aIndex"
									:class="{active: apm === a}"
